...
echo "----------Test for unambiguous hierarchical relations"
curl -s --data-urlencode query="$(< """$TESTDIR/broader_and_partof.query""")" -G "$FUSEKITESTDATASET/query" | grep '^\s*\"\(parent\|child\)\"' > "$errorfile"
errorcount=$(wc -l "$errorfile" | cut -d " " -f 1)
if [ $errorcount -gt 0 ]
then
		if [ ! -z "$errorsummary" ]; then 
			echo "Check for broader/narrower and hasPart/partOf: " >> "$errorsummary"
			sed -e "s/^.*\"value\":.\"\(.*\)\".*$/\1/" "$errorfile" | sed 's/\\/\\\\/g' | xargs -L 1 echo -e >> "$errorsummary"
		fi
        echo -e "${RED}These concepts have multiple hierarchical relations:${NC}"
        sed -e "s/^.*\"value\":.\"\(.*\)\".*$/\1/" "$errorfile" | sed "s/.*/\\${RED}\\0\\${NC}/" | sed 's/\\/\\\\/g' | xargs -L 1 echo -e
	EXITCODE=1
else
	echo -e "${GREEN}OK${NC}"
