...
export class ConfigurationService {
  ...
  public setConfigFromConfigFile(config:{}) {
    this.config$.next(config);
  }
  public getFusekiServer$():Observable<string> {
    return this.config$.pipe(
      map(c => c['fuseki_server'] + '/' + this.server_suffix)
    );
  }

  private server:'live'|'dev';
  private server_suffix='cometar_live';
  public setServer(s:'live'|'dev'){
    this.server = s;
    if (s === 'live') { this.server_suffix='cometar_live'; }
    if (s === 'dev') { this.server_suffix='cometar_dev'; }
  }
