...
export class DataService {
  constructor(
    ...
    this.http.get('./assets/config.json').subscribe(config => this.configurationService.setConfigFromConfigFile(config));
  }

  private pendings:{[index: string]: ReplaySubject<any[]>} = {};
  ...
  getData(queryString: string, queryName: string, typerules?: {}): Observable<any[]> {
    return this.configurationService.getFusekiServer$().pipe(
      flatMap(fuseki_url => {
        if (!this.pendings[queryString]) {
          ...
          s = this.getObservable(url, queryString, typerules).subscribe(data => {
            ...
            if (data instanceof HttpErrorResponse) {
              ...
              const tempSubject = new ReplaySubject<any[]>(1);
              tempSubject.next([]);
              this.pendings[queryString] = tempSubject;
            }
