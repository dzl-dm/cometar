...
export class DataService {
  ...
  public getDataSourcesData():ChartData{
    this.resetColors();
    const mappings:DataSourceMapping[] = Object.keys(mapping).map(key => mapping[key]).filter(m => m['first import'] != undefined).map(m => {
      if (typeof m['first import'] == 'string') {
        const d = m['first import'];
  ...
  private new_calculation_since_november_2019_filter(d:QpfddDataSet,datelabel:string,granularity:string):boolean{
    ...
    if (new Date(datelabel) < new Date('2019-11-15')) {
      ...
      const dfsnum = this.qpfddJson.filter(data => data.date == datelabel && data.source == d.source).map(data => data['distinct facts']);
    ...
    const datasets: ChartDataSets[] = baseArray.filter(b => {
      ...
      return {
        ...
        data: this.qpfddLabels.map(l => {
          const granularData = this.qpfddJson.filter(d => d.date == l && (
          ...
          const vcs = this.value_corrections.filter(vc => 
        ...
        lastUpdate: (()=>{
          const d = new Date(Math.max.apply(null,this.qpfddJson.filter(d => 
  ...
  private getBreakdownData(json:BreakdownDataSet[]):ChartData[]{
    const breakdownLabels = json.map(p => p.label).filter(this.distinctFilter);
    const breakdownSiteLabels = [];
    const breakdownData = [];
    const breakdownSiteData = [];
    const breakdownSiteColors = [];
  ...
  public getMultiPhenotypeData():ChartData{
    const multiPhenotypeData:ChartPoint[] = [];
    const percentageData1:ChartPoint[]=[];
    const percentageData2:ChartPoint[]=[];
    for (let i = 0; i < this.phenotypes.length; i++) {
      for (let j = 0; j < this.phenotypes.length; j++) {
        const phenocount = this.multiPhenotypeJson.filter(a => {
          const pts = <string[]>a['phenotypes'];
          return pts.includes(this.phenotypes[i]) && pts.includes(this.phenotypes[j]);
        }).length;
        const pbdd = this.getPhenotypeBreakdownData()[0];
        const xpbddIndex = pbdd.labels.indexOf(this.phenotypes[i]);
        let xPercentage = Math.round(phenocount / (<number>pbdd.datasets[0].data[xpbddIndex]) * 1000)/10;
        const ypbddIndex = pbdd.labels.indexOf(this.phenotypes[j]);
  ...
  public getI2b2UsageData():ChartData{
    const months = this.i2b2usageJson.map(i => i.month).filter(this.distinctFilter);
    ...
    const userspermonthDataset:ChartDataSets = {
    ...
    const queriesspermonthDataset:ChartDataSets = {
  ...
  constructor(
    ...
    const styleSubject = new ReplaySubject<TreeItemStyle[]>(1);
    this.ontologyAccessService.getAllTreeItems().subscribe(tis => {
      const styles = tis.map(ti => {
        const notations = ti.notations && ti.notations.value || [];
        const sources = codecoverage['data'].filter(a => notations.includes(a.concept_cd)).map(a => a.source);
        const style = this.treeStyleService.getEmptyStyle(ti.element.value);
        sources.forEach(source => {
          const map = mapping[source] && mapping[source]['same as'] && mapping[mapping[source]['same as']] || mapping[source] || {'site':'none','label':'not defined'}
  ...
  private getNewColor(colorAmount:number) {
    const colordiff = 208/colorAmount;
    this.colorcount++;
