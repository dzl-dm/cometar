...
import { Observable, combineLatest, ReplaySubject, BehaviorSubject, Subject, of } from 'rxjs';
...
export class TreeDataService {
  ...
  private showOnlyMatches$:BehaviorSubject<boolean> = new BehaviorSubject(false);
  private onlyHighlightTerms$:BehaviorSubject<boolean> = new BehaviorSubject(false);
  ...
  public init(route:ActivatedRoute, claimWidth:(number)=>void){
    ...
    route.queryParamMap.pipe(
      map(data => data.get('show_only_matches') ? data.get('show_only_matches') : "")
    ).subscribe(som => {
      this.showOnlyMatches$.next(som=="true");
    });
    route.queryParamMap.pipe(
      map(data => data.get('only_highlight_terms') ? data.get('only_highlight_terms') : "")
    ).subscribe(oht => {
      this.onlyHighlightTerms$.next(oht=="true");
    });
  ...
  public getShowOnlyMatches():BehaviorSubject<boolean>{
    return this.showOnlyMatches$;
  }
  public getOnlyHighlightTerms():BehaviorSubject<boolean>{
    return this.onlyHighlightTerms$;
  }
  ...
  public isSearchPathPart$(iri:string):Observable<boolean>{
    if (!this.showOnlyMatches$.getValue()) return of(true);
    return this.searchElements$.pipe(
      flatMap(cis => this.ontologyAccessService.getAllAncestors(cis).pipe(map(x => x.concat(cis)))),
      map(a => a.includes(iri))
    )
  }
}