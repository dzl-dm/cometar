...
export class MenuComponent implements OnInit {
  ...
  public showOnlyMatches$:BehaviorSubject<boolean> = new BehaviorSubject(false);
  public showOnlyMatches:boolean = false;
  public onlyHighlightTerms$:BehaviorSubject<boolean> = new BehaviorSubject(false);
  public onlyHighlightTerms:boolean = false;
  ...
  ngOnInit() {
    ...
    this.treeDataService.getShowOnlyMatches().subscribe(data => this.showOnlyMatches = data );
    this.treeDataService.getOnlyHighlightTerms().subscribe(data => this.onlyHighlightTerms = data );
  }
  ...
  public clickShowOnlyMatches(){
    if (!this.showOnlyMatches){
      this.router.navigate([],{ queryParams: {show_only_matches: null}, queryParamsHandling: "merge" });
    } else {
      this.router.navigate([],{ queryParams: {show_only_matches: "true"}, queryParamsHandling: "merge" });
    }
    return false;   
  }
  public clickOnlyHighlightTerms(){
    if (!this.onlyHighlightTerms){
      this.router.navigate([],{ queryParams: {only_highlight_terms: null}, queryParamsHandling: "merge" });
    } else {
      this.router.navigate([],{ queryParams: {only_highlight_terms: "true"}, queryParamsHandling: "merge" });
    }
    return false;   
  }
