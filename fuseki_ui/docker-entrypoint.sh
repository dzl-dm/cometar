...
if [ -n "$COMETAR_FUSEKI_ADMIN_PASSWORD" ] ; then
  ADMIN_PASSWORD=$COMETAR_FUSEKI_ADMIN_PASSWORD
  echo -e "Using ADMIN_PASSWORD from settings: $ADMIN_PASSWORD"
fi
...
if [ -n "$COMETAR_FUSEKI_JVM_ARGS" ] ; then
  JVM_ARGS=$COMETAR_FUSEKI_JVM_ARGS
  echo -e "Using JVM_ARGS from settings: $JVM_ARGS"
fi
